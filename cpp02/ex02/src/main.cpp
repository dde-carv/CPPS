...
/*   Updated: 2025/09/29 17:00:41 by dde-carv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main()
{
	...
	limit.setRawBits(2);
	x.setRawBits(0);
	std::cout << "--- PRE-ICREMENT OVERLOAD ---" << std::endl;
	while (++x < limit)
		std::cout << "x = " << x << " & limit = " << limit << std::endl;

	x.setRawBits(0);
	std::cout << std::endl << "--- POST-ICREMENT OVERLOAD ---" << std::endl;
	while (x++ < limit)
		std::cout << "x = " << x << " & limit = " << limit << std::endl;

	limit.setRawBits(0);
	x.setRawBits(2);
	std::cout << std::endl << "--- PRE-DECREMENT OVERLOAD ---" << std::endl;
	while (--x > limit)
		std::cout << "x = " << x << " & limit = " << limit << std::endl;

	x.setRawBits(2);
	std::cout << std::endl << "--- POST-DECREMENT OVERLOAD ---" << std::endl;
	while (x-- > limit)
		std::cout << "x = " << x << " & limit = " << limit << std::endl;
	...
	if ((y1 >= x1) && (y2 >= y1))
		std::cout << "(y1 >= x1) && (y2 >= y1)" << std::endl;
	if ((y1 <= x1) && (y1 <= y1))
		std::cout << "(y1 <= x1) && (y1 <= y2)" << std::endl;
	if (x1 == y1)
		std::cout << "x1 == y1" << std::endl;
	if (x1 != x2)
		std::cout << "x1 != x2" << std::endl << std::endl;
